...
"""
This module locates the R-waves, the Aortic valve closure (AVC) and the P-wave in the ECG data.
The peaks of the R-waves are detected automatically using "work_with_peaks" function.

The AVC is obtained by reading the respective excel file. If this file is not available, it is assumed to be the inverse
t wave in the ECG signal and it is located automatically using "find_inverse_t" function.
"""
...
def work_with_peaks(ecg, start, ending):
    ...
    # then there is no p-wave at the beginning of the signal and thus it is set to None
    ...
    # for the p-wave, if there isn't any R-wave is set as the highest peak in the second half of the ecg signal
